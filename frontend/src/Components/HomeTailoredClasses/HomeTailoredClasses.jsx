...
const HomeTailoredClasses = () => {
  ...
  return (
    <section className="tailored-classes-container" aria-labelledby="tailored-classes-heading">
      ...
      <header className={`tc-header ${isAnimate ? 'fade-in-up' : ''}`}>
        <div className="shape-worm" aria-hidden="true">
        ...
        <h1 id="tailored-classes-heading" className="tc-title">
          best kids school in bhubaneswar
        </h1>

        <p className="tc-intro">
          At Nanda Kidz, we believe every child deserves a cheerful, safe, and inspiring start to their educational journey. Our engaging activities blend joyful play with foundational growth.
        </p>
        ...
        {cardData.map((item, index) => (
          ...
          >
            ...
            <div className="tc-image-wrapper">
              ...
              <img src={item.image} alt={item.title} className="tc-card-image" loading="lazy" />
              ...
              <div className="tc-hover-actions">
                <button type="button" className="action-btn check-btn" title="Approve" aria-label="Approve">
                  <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" width="20" height="20">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M5 13l4 4L19 7" />
                  </svg>
                </button>
                <button type="button" className="action-btn view-btn" title="Quick View" aria-label="Quick View">
                  <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" width="20" height="20">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                  </svg>
                </button>
                <button type="button" className="action-btn wishlist-btn" title="Add to Wishlist" aria-label="Add to Wishlist">
            ...
            <div className="tc-card-content">
              <h2 className="tc-card-title">{item.title}</h2>
              <p className="tc-card-desc">{item.description}</p>

              {/* Dynamic Action Buttons based on state */}
              {item.isSoldOut ? (
                <button type="button" className="tc-btn btn-sold-out" disabled>
                  SOLD OUT
                  <span className="arrow-icon">↗</span>
                </button>
              ) : (
                <button type="button" className="tc-btn btn-add-cart">
              ...
              <div className="tc-rating-container">
                <div className="stars-row" aria-label={`${item.reviews} out of 5 stars`}>
                  {item.stars.map((filled, starIdx) => (
                    <span
                      key={starIdx}
                      className={`star ${filled ? 'filled' : 'empty'}`}
                      style={{ animationDelay: `${starIdx * 0.06}s` }}
                    >
                      ★
                    </span>
                  ))}
      ...
      {/* SEO Supporting Context Section */}
      <div className="tc-seo-section">
        <div className="tc-seo-line"></div>
        <div className="tc-seo-text-block">
          <h3>Finding a top best nursery school in bhubaneswar</h3>
          <p>
            When parents evaluate early education options, finding a nurturing atmosphere that balances structured routines with creative exploration is essential. Nanda Kidz provides a supportive framework where children build communication skills, foundational confidence, and deep social bonds naturally.
          </p>
        </div>
      ...
      <div className="tc-footer-actions">
        <button type="button" className="tc-btn-view-all">
          <span className="tc-btn-shine"></span>
